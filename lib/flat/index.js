...
const processOne = function (flow, ...data) {
	...
	if (item.task instanceof Lightflow) {
		item.task.done((...args) => process(flow, ...args))['catch'](err => processError(flow, err)).loop(false).start(...data);
	} else {
		item.task.call(item.context, (...args) => process(flow, ...args), err => processError(flow, err), ...data);
	}
};
...
const processMany = function (flow, ...data) {
	...
	item.task.forEach((task, idx) => {
		task.task.call(task.context, max => counter[idx] = max, (...args) => {
			if (--counter[idx] >= 0) {
				ret.push(args);
			}
		...
		}, err => processError(flow, err), ...data);
	});
};
