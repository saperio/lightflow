...
const process = function process(flow, data) {
	if (!flow.active) {
...
const processInternal = function processInternal(flow, data) {
...
const processOne = function processOne(flow, data) {
	var _flow$taskChain$flow$ = flow.taskChain[flow.idx];
	const task = _flow$taskChain$flow$.task;
	const context = _flow$taskChain$flow$.context;

	task.call(context, {
		next: nextData => process(flow, nextData),
		error: err => processError(flow, err),
		data
	});
};

const processMany = function processMany(flow, data) {
...
class Lightflow {
	...
	then(task, context) {
		...
		if (task instanceof Lightflow) {
			task.done(nextData => process(this, nextData))['catch'](err => processError(this, err));
